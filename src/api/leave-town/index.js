...
exports.handler = function(event, context, callback) {
    ...
    LoginDAO.getByToken(requestInput.accessToken, (err, loginDTO) => {
        if(err) { Logger.logError(err); HttpController.respondError(origin, 500, "Failed to leave town(1):" + err, callback); return; }
        HeroDAO.get(loginDTO.userGuid, loginDTO.activeHeroName, (err, heroDTO) => {
            if(err) { Logger.logError(err); HttpController.respondError(origin, 500, "Failed to leave town(2):" + err, callback); return; }
            var hero = new Hero(heroDTO);
            hero.leaveTown(loginDTO, heroDTO, (err, data) => {
                if(err) { Logger.logError(err); HttpController.respondError(origin, 500, "Failed to leave town(3):" + err, callback); return; }
